...
#define channel_A   0x30 // DAC channel_A
#define channel_B   0x34 // DAC channel_B
...
void ADS1256_ISR(void)
{
	if (g_tADS1256.ScanMode == 0)	/*  0  Single-ended input  8 channel£¬ 1 Differential input  4 channe */
	{
		...
		{
			g_tADS1256.AdcNow[1] = ADS1256_ReadData();	
		}
		else
		{
...
int  main()
{
    ...
    int32_t adc[2];
    int32_t volt[2];
    uint16_t i, j, t,k, ciclos, pasos_rama,cambio_m;
    uint8_t del;
    uint8_t ch_num,factor,factor2;
    int32_t iTemp;
    uint8_t buf[3];
    FILE *fp;
    if (!bcm2835_init())
    ...
    int8_t multiplyer_scale=1;
	bcm2835_pwm_set_range(1, 1024*multiplyer_scale);	
	...
	ch_num = 2;
	factor = 1;
	factor2= 8*multiplyer_scale;
	ciclos = 10;
	cambio_m=(560*multiplyer_scale);
	pasos_rama =800*multiplyer_scale;
	int64_t contador;
	del = 0;
	int64_t field[pasos_rama*4], signal[pasos_rama*4];
	int64_t svoltage[pasos_rama*4], fvoltage[pasos_rama*4];
	//Write_DAC8532(0x30,0.5);
//	for (i=0;i<4000;i=i+1){
//			bcm2835_pwm_set_data(1,i);
//		    bsp_DelayUS(del);
//			while((ADS1256_Scan() == 0));
//			adc[0] = ADS1256_GetAdc(0);
//			adc[1] = ADS1256_GetAdc(1);
//		}
...
/*################################################################*/
		for (t=0;t<cambio_m;t=t+(factor2*factor)){
			bcm2835_pwm_set_data(1,t);
		    bsp_DelayUS(del);
			while((ADS1256_Scan() == 0));
			adc[0] = ADS1256_GetAdc(0);
			field[contador] = adc[0];
			adc[1] = ADS1256_GetAdc(1);
			signal[contador] = adc[1];
			contador = contador + factor;
		}

		for (t=cambio_m;t<pasos_rama;t=t+factor){
			bcm2835_pwm_set_data(1,t);
			bsp_DelayUS(del);
	    	while((ADS1256_Scan() == 0));
			adc[0] = ADS1256_GetAdc(0);
		...
		for (t=pasos_rama;t>cambio_m;t=t-factor){
			bcm2835_pwm_set_data(1,t);
	    	bsp_DelayUS(del);
		    while((ADS1256_Scan() == 0));
			adc[0] = ADS1256_GetAdc(0);
		...
		//fprintf(fp, "###################\n" );
		for (t=cambio_m;t>0;t=t-(factor2*factor)){
			bcm2835_pwm_set_data(1,t);
	    	bsp_DelayUS(del);
		    while((ADS1256_Scan() == 0));
			adc[0] = ADS1256_GetAdc(0);
...
/*################################################################*/
		...
		//fprintf(fp, "###################\n" );
		for (t=cambio_m;t>0;t=t-(factor*factor2)){
			bcm2835_pwm_set_data(1,t);
		    bsp_DelayUS(del);
		    while((ADS1256_Scan() == 0));
			adc[0] = ADS1256_GetAdc(0);
...
//		bcm2835_gpio_write(6, HIGH);
//		bcm2835_gpio_write(5, LOW);
//		for (t=0;t<cambio_m;t=t+(factor*factor2)){
//			bcm2835_pwm_set_data(1,t);
//		    bsp_DelayUS(del);
//			while((ADS1256_Scan() == 0));
//			adc[0] = ADS1256_GetAdc(0);
...
//		}
        clock_t end = clock();
//Impresion
		for (i = 0; i <(( pasos_rama-cambio_m)+cambio_m/factor2) * 4; ++i)
		{
			fvoltage[i] = (field[i] * 100) / 167;
			iTemp = fvoltage[i];
			fprintf(fp,"%ld.%03ld%03ld\t", iTemp /1000000, (iTemp%1000000)/1000, iTemp%1000);
			svoltage[i] = (signal[i] * 100) / 167;
			iTemp = svoltage[i];
			fprintf(fp,"%ld.%03ld%03ld\t", iTemp /1000000, (iTemp%1000000)/1000, iTemp%1000);
			fprintf(fp, "\n");
		}
